...
for (i = 0; i < countTwo; i++) {
  var responseSeven = prompt('Can you guess my favorite colors?');

  for(var j = 0; j < myFavoriteColors.length; j++) {
    if (responseSeven === myFavoriteColors[j]) {
      alert(`Yes, ${myFavoriteColors[j]} is one of my favorite colors`);
      rightAnswer = true;
    }
  }
  if (rightAnswer){
